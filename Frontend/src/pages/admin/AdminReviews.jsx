      ...
      {items.map((r) => (
        <div key={r.id} className={`card p-5 ${!r.is_approved ? 'opacity-60' : ''}`}>
          ...
          <div className="mt-4 flex gap-2 border-t border-line pt-3">
            <button onClick={() => toggleFeatured(r)} className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium ${r.is_featured ? 'bg-gold/15 text-gold' : 'bg-white/5 text-muted hover:bg-white/10'}`}>
              <Award size={13} /> {r.is_featured ? 'Featured' : 'Feature'}
            </button>
            <button onClick={() => toggleApproved(r)} className={`rounded-lg px-3 py-1.5 text-xs font-medium ${r.is_approved ? 'bg-emerald-400/15 text-emerald-300 border border-emerald-400/25' : 'bg-white/10 text-muted border border-line'}`}>
