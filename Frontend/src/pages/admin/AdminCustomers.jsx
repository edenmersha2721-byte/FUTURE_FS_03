              ...
              {items.map((c) => (
                <tr key={c.id} className="border-t border-sand/60">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${c.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-200 text-gray-600'}`}>{c.is_active ? 'Active' : 'Inactive'}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => view(c.id)} className="rounded-lg p-2 text-blue-600 hover:bg-blue-50"><Eye size={15} /></button>
                      <button onClick={() => toggle(c)} className={`rounded-lg p-2 ${c.is_active ? 'text-rose-500 hover:bg-rose-50' : 'text-emerald-600 hover:bg-emerald-50'}`}>
        ...
        {detail && (
          <div>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-serif text-xl text-charcoal">{detail.name}</p>
                <p className="text-sm text-muted">{detail.email} · {detail.phone || 'no phone'}</p>
              </div>
            </div>
            <h4 className="mt-6 font-medium text-espresso">Booking History ({detail.appointments.length})</h4>
            <div className="mt-2 max-h-72 space-y-2 overflow-auto">
              {detail.appointments.length === 0 && <p className="text-sm text-muted">No bookings yet.</p>}
              {detail.appointments.map((a) => (
                <div key={a.id} className="flex items-center justify-between rounded-xl border border-sand p-3 text-sm">
                  <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-gold-dark">{currency(a.price_snapshot)}</span>
