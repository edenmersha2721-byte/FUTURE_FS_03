...
export default function SectionHeading({ eyebrow, title, subtitle, center = true, light = false }) {
  return (
    <Reveal className={center ? 'text-center' : ''}>
      ...
      <h2 className={`section-title ${light ? 'text-cream' : 'text-charcoal'}`}>{title}</h2>
      ...
      {subtitle && (
        <p className={`mx-auto mt-4 max-w-2xl text-base leading-relaxed ${light ? 'text-cream/70' : 'text-muted'}`}>
          {subtitle}
        </p>
      )}
    </Reveal>
